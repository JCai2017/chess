...
public class BlackPawn extends ChessPieces {
	...
	public boolean isValidMove(int xPos, int yPos){
		...
		if(start){
			if(yPos==this.y+2 && xPos == this.x && board.posValid(xPos, yPos)){
				start=false; 
				moveTwoPlaces=true;
				return true;
			}
		...
		if(yPos==this.y+1 && xPos == this.x &&  board.posValid(xPos, yPos)){
			...
			if((!board.posValid(xPos, yPos)) && (board.positions.get(xPos).get(yPos).isWhite())){
				...
				start=false; 
				return true;
			}else if((!board.posValid(xPos, yPos + 1)) && (board.positions.get(xPos).get(yPos) instanceof WhitePawn)){
				WhitePawn take = (WhitePawn)(board.positions.get(xPos).get(yPos));
				if(take.movedTwo()){
					removePiece(take);
					start=false; 
					return true;
	...
	Image piece() {
		Image image = new Image("blackPawn.png");
		return image;
	}
